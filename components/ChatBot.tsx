...
const ChatBot: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'bot', text: 'Hi! I\'m the HireSkillr AI Assistant. Ask me anything about our services.' }
  ]);
  ...
  return (
    <>
      ...
      <button
        onClick={() => setOpen(o => !o)}
        className={`fixed bottom-4 right-4 2xs:bottom-6 2xs:right-6 z-50 w-12 h-12 2xs:w-14 2xs:h-14 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-white shadow-lg flex items-center justify-center hover:scale-110 transition-transform duration-200 ${open ? 'scale-0' : 'scale-100'}`}
      ...
      <div className={`fixed z-50 flex flex-col shadow-2xl overflow-hidden backdrop-blur-xl bg-white/95 dark:bg-black/95 border border-gray-200 dark:border-gray-800 transition-all duration-300 origin-bottom-right bottom-0 right-0 w-full h-[100dvh] rounded-none xs:bottom-4 xs:right-4 xs:w-[calc(100%-2rem)] xs:h-[70vh] xs:rounded-2xl sm:bottom-6 sm:right-6 sm:w-[380px] sm:h-[500px] ${open ? 'scale-100 opacity-100' : 'scale-0 opacity-0 pointer-events-none'}`}>
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-indigo-500 to-purple-600 text-white">
          <span className="font-bold text-sm">HireSkillr AI Assistant</span>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] px-3 py-2 rounded-xl text-sm ${m.role === 'user' ? 'bg-gradient-to-br from-indigo-500 to-purple-600 text-white' : 'bg-gray-200 dark:bg-gray-800 text-black dark:text-white'}`}>
                {m.text}
              </div>
            </div>
          ))}
        ...
        <div className="flex items-center gap-2 p-3 pb-safe-bottom border-t border-gray-200 dark:border-gray-800">
          ...
          <input
            ...
            placeholder="Ask about HireSkillr AI..."
            className="flex-1 bg-transparent outline-none text-sm text-black dark:text-white placeholder-gray-400 min-h-[40px]"
          />
          <button onClick={sendMessage} disabled={loading || !input.trim()} className="p-2.5 min-w-[40px] min-h-[40px] flex items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-white disabled:opacity-50 transition-opacity" aria-label="Send">
