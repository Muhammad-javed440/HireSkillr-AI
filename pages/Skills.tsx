...
const categoryColors: Record<string, string> = {
  ...
  'DevOps': 'from-yellow-500 to-violet-500',
  ...
  return (
    <div className="py-8 sm:py-12 md:py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="text-center mb-10 sm:mb-16 space-y-4 sm:space-y-6">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 bg-violet-600/10 border border-violet-500/20 rounded-full text-violet-600 text-xs sm:text-sm font-bold uppercase tracking-wider">
          <Award className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
          Our Expertise
        </div>
        <h1 className="text-3xl 2xs:text-4xl md:text-6xl font-black text-black dark:text-white uppercase tracking-tighter">
          SKILLS & <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600">TECHNOLOGIES</span>
        </h1>
            ...
            <div key={category} className="space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                {categorySkills.map(skill => (
                  <div
                    key={skill.id}
                    className="group bg-white dark:bg-black border-2 border-gray-100 dark:border-gray-900 rounded-2xl sm:rounded-3xl p-4 sm:p-6 hover:border-violet-500/50 transition-all duration-300 hover:shadow-xl hover:shadow-violet-600/5"
                  >
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        {skill.icon ? (
                          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-violet-500/10 to-purple-600/10 flex items-center justify-center text-2xl">
                            {skill.icon}
                          </div>
                        ) : (
      ...
      {skills.length > 0 && (
        <div className="mt-12 sm:mt-20 grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-6">
          <div className="bg-gradient-to-br from-violet-500 to-purple-600 rounded-2xl sm:rounded-3xl p-5 sm:p-8 text-center text-white">
